...
public class JINuevaVenta extends javax.swing.JInternalFrame {
    ...
    private void btnPagarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPagarActionPerformed
        int pago, nvc, idCliente, respNVC, saldoLinea;
        float cambio;
        ...
        try {
            Connection cn = Conexion.conectar();
            PreparedStatement pst = cn.prepareStatement("select NC_Cliente_idNC_Cliente, LimiteCredito from clientes, lc_cliente where idClientes = '" + idCliente + "' "
                    + "and LC_Cliente_idNC_Cliente = '" + idCliente + "'");
            ResultSet rs = pst.executeQuery();

            if (rs.next()) {
                nvc = rs.getInt("NC_Cliente_idNC_Cliente");
